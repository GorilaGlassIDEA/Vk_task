...
public class TestFullGrpc {
    public static void main(String[] args) {
        ...
        // 2. Добавляем несколько записей через put
        System.out.println("\n2. Добавляем записи:");

        stub.put(Api.PutRequest.newBuilder()
                .setKey("user1")
                .setValue(com.google.protobuf.ByteString.copyFromUtf8("Alice"))
                .build());
        System.out.println("   - Добавлен user1 = Alice");

        stub.put(Api.PutRequest.newBuilder()
                .setKey("user2")
                .setValue(com.google.protobuf.ByteString.copyFromUtf8("Bob"))
                .build());
        System.out.println("   - Добавлен user2 = Bob");

        stub.put(Api.PutRequest.newBuilder()
                .setKey("user3")
                .setValue(com.google.protobuf.ByteString.copyFromUtf8("Charlie"))
                .build());
        System.out.println("   - Добавлен user3 = Charlie");
        ...
        // 4. Получаем значения через get
        System.out.println("\n4. Получение значений:");

        Api.GetResponse get1 = stub.get(Api.GetRequest.newBuilder().setKey("user1").build());
        System.out.println("   - user1: " + (get1.getFound() ? get1.getValue().toStringUtf8() : "не найден"));

        Api.GetResponse get2 = stub.get(Api.GetRequest.newBuilder().setKey("user2").build());
        System.out.println("   - user2: " + (get2.getFound() ? get2.getValue().toStringUtf8() : "не найден"));

        Api.GetResponse get3 = stub.get(Api.GetRequest.newBuilder().setKey("user3").build());
        System.out.println("   - user3: " + (get3.getFound() ? get3.getValue().toStringUtf8() : "не найден"));

        // 5. Обновляем значение
        System.out.println("\n5. Обновление значения:");
        stub.put(Api.PutRequest.newBuilder()
                .setKey("user2")
                .setValue(com.google.protobuf.ByteString.copyFromUtf8("Robert"))
                .build());
        System.out.println("   - user2 обновлен на Robert");
